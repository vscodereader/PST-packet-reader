...
로그인 자동화가 실제로 동작하는지 **개발자가 점검**하는 방법이다. 글쓰기/댓글 자동화는
OS 무관하게 이미 양쪽에서 동작하므로, 이 문서는 **로그인 자동화** 검증에 집중한다.

> **중요 — 쿠키 경로·Chrome 경로는 직접 지정하지 않는다.**
> 실제 배포 앱과 아래 "방법 A 예제"에서는 사용자가 **계정 ID와 비밀번호만** 입력하면 되고,
> 쿠키 저장 경로와 Chrome 경로는 **앱(또는 예제)이 자동으로 계산**한다
> (`src-tauri/src/auth/playwright.rs`가 처리). 사용자가 경로를 만질 일은 없다.
> 아래 "방법 B"는 바이너리만 따로 떼서 저수준으로 점검하는 **선택적/디버그용**이며,
> 그때만 입력 JSON에 경로가 보인다.
...
## 1. 방법 A — 예제로 테스트 (권장, 가장 간단)

`src-tauri/examples/auto_login.rs`가 입력을 만들어 sidecar 바이너리를 실행한다.
**쿠키 경로·Chrome 경로는 예제가 자동으로 정한다** — 당신은 ID/PW만 넣으면 된다.
...
## 2. 방법 B — 바이너리를 직접 실행 (선택, 저수준 디버그용)

> 보통은 할 필요 없다. 앱과 예제는 이 입력 JSON을 자동으로 만든다. 바이너리 자체만 따로
> 점검하고 싶을 때만 사용한다. 이때만 `cookiesPath`·`chromePath`를 손으로 적는다.
