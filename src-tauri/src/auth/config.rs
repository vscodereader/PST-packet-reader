...
// ADB
/// 비행기모드 토글 상태(ON/OFF)가 ADB(`settings get global airplane_mode_on`)로 확정될 때까지
/// 폴링하는 간격(ms). 고정 대기 대신 실제 상태를 확인해 확정 즉시 다음으로 넘어간다(사수 지시).
pub const ADB_AIRPLANE_CONFIRM_POLL_MS: u64 = 100;
/// 비행기모드 상태 확정 폴링의 상한(초). 상태를 확인 못 해도 무한 대기하지 않도록 둔다 —
/// 넘으면 토글 명령은 이미 실행됐으므로 그대로 진행한다.
