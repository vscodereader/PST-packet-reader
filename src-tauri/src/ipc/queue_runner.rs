            ...
            CommentTarget::Latest | CommentTarget::Popular => {
                ...
                let fetched = match spec.mode {
                    CommentTarget::Latest => {
                        fetch_latest_articles_for_account_up_to(&cafe_str, &t.account_id, count).await
                    }
                    _ => fetch_article_list_for_account(&cafe_str, SortBy::Popular, 1, &t.account_id)
                        .await
...
/// 공용). 순서는 글→댓글→조회 실패→종토방→밴드로 고정한다.
fn build_items(
    plan: &PublishPlan,
    post_reports: &[JobReport],
