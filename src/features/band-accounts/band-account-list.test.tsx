...
import { BandAccountList } from "./band-account-list";

describe("BandAccountList", () => {
  it("renders the heading", () => {
    render(<BandAccountList />);
    expect(
      screen.getByRole("heading", { name: /band account manager/i }),
    ).toBeInTheDocument();
  });
  ...
  it("calls save_accounts and enqueue_band_login when run-all is clicked", async () => {
    ...
    await waitFor(() => {
      ...
      expect(invoke).toHaveBeenCalledWith("enqueue_band_login", {
        accountIds: ["myid"],
        headless: false,
        useAdb: false,
      });
  ...
  it("shows a login error when enqueue rejects", async () => {
    ...
    vi.mocked(invoke)
      ...
      .mockRejectedValueOnce(new Error("로그인 시작 실패")); // enqueue_band_login
    const user = userEvent.setup();
    render(<BandAccountList />);
    await user.type(screen.getByPlaceholderText("Username"), "myid");
    await user.type(screen.getByPlaceholderText("Password"), "mypass");
